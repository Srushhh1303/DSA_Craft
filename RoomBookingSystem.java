...
public class RoomBookingSystem {
    ...
    private static boolean authenticateAdmin() {
        ...
        if (adminUsername.equals("BNY") && adminPassword.equals("bny1")) {
    ...
    private static boolean authenticateUser() {
        ...
        if (userUsername.equals("abc") && userPassword.equals("abc1")) {
    ...
    private static void addConferenceRoom() {
        ...
        System.out.println("Enter location (floor) of the conference room (1,5,7):");
        int floor = Integer.parseInt(scanner.nextLine());
       if(floor == 1 || floor==5 || floor==7)
       {
        // Use the current roomIdCounter value when creating the ConferenceRoom object
        conferenceRooms.put(roomIdCounter, new ConferenceRoom(roomIdCounter, name, capacity, floor, true));
        roomIdCounter++; // Increment the counter for the next room
        System.out.println("Conference room added successfully.");
       }
       else
       {
    	   System.out.println("No Conference room availabel on entered floor");
       }
    }
